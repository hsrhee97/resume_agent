"""
llm_client.py — HuggingFace Inference Providers / Ollama
...
class BaseLLM(ABC):
    ...
    def _parse_json(text: str) -> dict | None:
        ...
        try: return json.loads(text)
            ...
            try: return json.loads(text)
            except: print("  ⚠️ JSON 파싱 실패"); return None
...
class HuggingFaceLLM(BaseLLM):
    ...
    def _select_model(self, preferred: str = None):
        ...
        print("🔍 사용 가능한 모델 탐색 중 (chat_completion)...")
            ...
            try:
                ...
                if attempt < 2:
                    ...
                    if "429" in err:
                        ...
                        wait = 15
                        print(f"  ⏳ 모델 로딩 중. {wait}초 대기...")
                    else:
                        print(f"  🔄 Retry {attempt+1}: {err[:60]}")
                    time.sleep(wait)
                else:
                    print(f"  ❌ 최종 실패: {err[:100]}")
        return ""
...
class OllamaLLM(BaseLLM):
    ...
    def __init__(self, model="qwen2.5", base_url="http://localhost:11434"):
        ...
        try:
            ...
            print(f"  {'✅' if any(model in m for m in ms) else '⚠️'} Ollama: {model}")
        except: print("  ❌ Ollama 미실행 → ollama serve")
    ...
    def generate(self, prompt: str, max_tokens: int = 4000) -> str:
        try:
            ...
            print(f"  ❌ Ollama: {str(e)[:60]}"); return ""