...
ESSAY_WRITING_PROMPT = PromptTemplate.from_template(
    """
너는 한국어 자기소개서 작성 코치다.
아래 입력 JSON 기반 컨텍스트를 근거로 4개 문항의 제출용 완성본을 작성하라.
...
3. 문체는 일관된 1인칭 서술형으로 작성하고, 문장은 단정하고 전문적으로 쓴다.
4. 각 문항은 8~12문장으로 작성한다.
5. 각 문항은 공백 제외 350자 이상 600자 이하로 작성한다.
6. 각 문항은 다음 구조를 자연스럽게 포함한다: 도입(지원 맥락) -> 근거 경험(행동) -> 결과/배운점 -> 지원 직무 연결 -> 결론.
7. 입력 JSON에 있는 경우에만 정량 근거(수치/성과)를 사용한다. 없으면 임의로 만들지 않는다.
8. 초안 표현(예: "추후 보완", "작성 예정", "더 채워 넣을")은 금지한다.
9. 설명 문장 없이 JSON만 출력한다.
10. [추가 작성 지시]가 "없음"이 아니면 해당 지시를 최우선으로 반영하되, 사실 근거 제한 규칙을 반드시 지킨다.
